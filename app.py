...
@app.route("/financial_info", methods=["GET", "POST"])
def financial_info():
    stock_code = session.get("stock_code")
    stock = yf.Ticker(stock_code)
    company_name = stock.info.get("longName", "N/A")
...
@app.route("/genAI_index", methods=["GET", "POST"])
def genAI_index():
    return render_template("genAI_index.html", r=None)

@app.route("/genAI_result_index", methods=["POST"])
...
@app.route("/genAI_result", methods=["POST"])
def genAI_result():
    q = request.form.get("q")
    r = model.generate_content(q).candidates[0].content.parts[0].text 
    r_html = markdown.markdown(r)
...
@app.route("/investment_result", methods=["GET", "POST"])
